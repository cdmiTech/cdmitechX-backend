...
exports.getFacultyReports = async (req, res) => {
    try {
        ...
        if (studentId) {
            ...
            // Return all reports for this student so any viewing faculty/admin can see full history & CPC progress
            ...
            if (facultyId && facultyId !== 'all') {
                ...
                // All faculties, no filter on facultyId
            } else if (facultyId === undefined && req.user.role === 'faculty') {
        ...
        if (date) {
            ...
            const submittedStudentIds = reports.map(r => (r.studentId?._id || r.studentId)?.toString()).filter(Boolean);
