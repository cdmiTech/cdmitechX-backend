    ...
    try {
        ...
        if (file) {
            ...
            const result = await uploadImage(
                file.buffer,
                'workbook/materials',
                file.originalname ? file.originalname.replace(/\.[^/.]+$/, "") : name,
                name,
                file.mimetype,
                file.originalname
            );
...
// @desc    Append file/PDF to existing material
...
exports.appendPDF = async (req, res) => {
    try {
        ...
        // Check for duplicate sub-material name if name provided
        ...
        for (const file of files) {
            let finalName = name || file.originalname || `File-${Date.now()}`;
            if (name && index > 0) {
            ...
            const result = await uploadImage(
                file.buffer,
                'workbook/materials',
                file.originalname ? file.originalname.replace(/\.[^/.]+$/, "") : finalName,
                finalName,
                file.mimetype,
                file.originalname
            );
...
// @desc    Delete single file/PDF from material
...
exports.deletePDF = async (req, res) => {
    try {
        ...
        const targetPdf = material.pdfs.find(pdf => pdf.public_id === public_id);

        // Remove from Cloudinary with matching resource_type
        try {
            if (targetPdf && targetPdf.resource_type) {
                await cloudinary.uploader.destroy(public_id, { resource_type: targetPdf.resource_type });
            } else {
                const res1 = await cloudinary.uploader.destroy(public_id, { resource_type: 'image' });
                if (res1.result !== 'ok') {
...
exports.deleteMaterial = async (req, res) => {
    try {
        ...
        for (const pdf of material.pdfs) {
            try {
                const rType = pdf.resource_type || 'image';
                await cloudinary.uploader.destroy(pdf.public_id, { resource_type: rType });
            } catch (cErr) {
